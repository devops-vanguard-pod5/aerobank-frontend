...
const ViewAllBankTransactions = () => {
  ...
  const retrieveAllTransactions = async () => {
    if (!admin_jwtToken) {
      toast.error("Admin session expired. Please log in again.", { position: "top-center", autoClose: 2000 });
      navigate("/user/login");
      return null;
    }
    ...
    try {
      const response = await axios.get(
        "https://bankapi.devopsvanguard.online/api/bank/transaction/all",
        {
      ...
      if (error.response && error.response.status === 403) {
        ...
        toast.error("Failed to load transactions. Server might be down.");
      }
      return null;
    }
  };
      ...
      <div className="mt-2">
        <div className="card form-card ms-5 me-5 mb-5 custom-bg border-color shadow" style={{ height: "45rem" }}>
          <div className="card-header custom-bg-text text-center bg-color">
            <h2>All System Transactions (Admin)</h2>
          </div>
